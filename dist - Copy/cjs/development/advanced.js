...
var advanced_events_exports = {};
__export(advanced_events_exports, {
  enableAdvancedEvents: () => enableAdvancedEvents,
  rescanCommandOnElements: () => rescanCommandOnElements
});
module.exports = __toCommonJS(advanced_events_exports);
...
function handleTrigger(event) {
  ...
  const triggerAttr = source.getAttribute("command-on") || source.dataset.onEvent || source.originalTriggerAttr;
  const allModifiers = triggerAttr.split(".").slice(1);
  const modifiers = allModifiers.filter((m) => m !== "window");
  if (event.type === "keydown" || event.type === "keyup") {
    const keyboardEvent = event;
    const keyModifier = modifiers.find((mod) => {
  ...
  const interpolationContext = {
    ...
    this: source,
    target: targetElement,
    detail: event == null ? void 0 : event.detail
  };
  const interpolatedCommand = interpolateString(commandAttribute, interpolationContext);
  _dispatchCommandEvent(source, interpolatedCommand, targetElement, event);
...
function attachListeners(element) {
  if (element.hasAttribute("command-on") && !element.dataset.commandOnAttached) {
    ...
    const listener = (event) => handleTrigger.call(element, event);
    target.addEventListener(eventName, listener);
    element.dataset.commandOnAttached = "true";
    element.originalTriggerAttr = triggerAttr;
    element.commandOnListener = listener;
  }
...
function disconnectListeners(element) {
  if (element.dataset.commandOnAttached) {
    const triggerAttr = element.originalTriggerAttr;
    const parts = triggerAttr.split(".");
    const eventName = parts[0];
    const modifiers = parts.slice(1);
    const target = modifiers.includes("window") ? window : element;
    const listener = element.commandOnListener;
    target.removeEventListener(eventName, listener);
    delete element.dataset.commandOnAttached;
    delete element.originalTriggerAttr;
    delete element.commandOnListener;
  }
    ...
    if (mutation.type === "childList") {
      mutation.addedNodes.forEach((node) => {
        if (node.nodeType === 1) {
          ...
          element.querySelectorAll("[command-on], [data-on-event]").forEach(attachListeners);
        }
      });
      mutation.removedNodes.forEach((node) => {
        if (node.nodeType === 1) {
          ...
          element.querySelectorAll("[command-on], [data-on-event]").forEach(disconnectListeners);
        }
      });
    } else if (mutation.type === "attributes") {
      const element = mutation.target;
      if (mutation.attributeName === "command-on" && element.hasAttribute("command-on") && !element.dataset.commandOnAttached || mutation.attributeName === "data-on-event" && element.hasAttribute("data-on-event") && !element.dataset.onEventAttached) {
        attachListeners(element);
      } else if (mutation.attributeName === "command-on" && !element.hasAttribute("command-on") && element.dataset.commandOnAttached || mutation.attributeName === "data-on-event" && !element.hasAttribute("data-on-event") && element.dataset.onEventAttached) {
        disconnectListeners(element);
      }
    }
  }
});
var EventTriggerManager = class _EventTriggerManager {
  static getInstance() {
    if (!_EventTriggerManager.instance) {
      _EventTriggerManager.instance = new _EventTriggerManager();
    }
    return _EventTriggerManager.instance;
  }
  initialize(root = document.body) {
    root.querySelectorAll("[command-on], [data-on-event]").forEach(attachListeners);
  ...
  shutdown() {
    observer.disconnect();
    document.querySelectorAll("[command-on][data-command-on-attached], [data-on-event][data-on-event-attached]").forEach(disconnectListeners);
    console.log("Invokers EventTriggerManager shut down.");
  }
  rescanCommandOnElements(root = document.body) {
    root.querySelectorAll("[command-on], [data-on-event]").forEach(attachListeners);
  }
};

// src/advanced-events.ts
function enableAdvancedEvents() {
  const invokerInstance2 = InvokerManager.getInstance();
  invokerInstance2._enableInterpolation();
  if (typeof window !== "undefined" && window.Invoker) {
    window.Invoker.getInterpolationUtility = () => interpolateString;
    window.Invoker.interpolateString = interpolateString;
    window.Invoker.generateUid = generateUid;
  }
...
function rescanCommandOnElements() {
