...
var demo_commands_exports = {};
__export(demo_commands_exports, {
  demoCommands: () => demoCommands,
  registerDemoCommands: () => registerDemoCommands
});
module.exports = __toCommonJS(demo_commands_exports);
...
var demoCommands = {
   ...
   */
  "--demo:echo": ({ targetElement }) => {
    targetElement.textContent = `Echo: ${(/* @__PURE__ */ new Date()).toISOString()}`;
  },
  /**
   * `--demo:counter:increment`: Demo counter increment command.
   * @example `<button command="--demo:counter:increment" data-max-count="10">`
   */
  "--demo:counter:increment": ({ invoker, targetElement }) => {
   ...
   */
  "--demo:counter:reset": ({ targetElement }) => {
   ...
   */
  "--demo:fetch:api": async ({ invoker, targetElement }) => {
   ...
   */
  "--demo:fetch:simulate-error": async ({ targetElement }) => {
    targetElement.innerHTML = '<div style="text-align: center; padding: 20px;"><div class="status-indicator status-loading pulse"></div>Loading...</div>';
   ...
   */
  "--demo:chain:start": async ({ targetElement }) => {
   ...
   */
  "--demo:event:publish": ({ invoker, targetElement }) => {
...
`;
   ...
   */
  "--demo:chart:add-point": ({ invoker, targetElement }) => {
    const valueSource = invoker.dataset.valueFrom;
    let value = Math.random() * 100;
    if (valueSource) {
      const input = document.getElementById(valueSource);
      if (input && input.value) {
        value = parseFloat(input.value) || value;
      }
    }
    const demoChart = window.demoChart;
    if (demoChart) {
      demoChart.data.labels.push((/* @__PURE__ */ new Date()).toLocaleTimeString());
   ...
   */
  "--demo:chart:clear": ({ targetElement }) => {
    const demoChart = window.demoChart;
    if (demoChart) {
   ...
   */
  "--demo:api:github-user": async ({ invoker, targetElement }) => {
   ...
   */
  "--demo:template:load-list": ({ targetElement }) => {
    const sampleData = [
   ...
   */
  "--demo:condition:update": ({ invoker, targetElement }) => {
    const condition = invoker.value;
    let content = "";
    switch (condition) {
      case "loading":
   ...
   */
  "--demo:programmatic": ({ targetElement }) => {
    targetElement.textContent += `Programmatic execution at ${(/* @__PURE__ */ new Date()).toLocaleTimeString()}
`;
  },
  /**
   ...
   */
  "--demo:delayed": ({ targetElement }) => {
    targetElement.textContent += `Delayed execution completed at ${(/* @__PURE__ */ new Date()).toLocaleTimeString()}
`;
  },
  /**
   ...
   */
  "--demo:queue:start": async ({ targetElement }) => {
...
`;
   ...
   */
  "--demo:error:network": (_) => {
    throw createInvokerError("Network connection failed", "error" /* ERROR */, {
   ...
   */
  "--demo:error:validation": (_) => {
    throw createInvokerError("Invalid input data provided", "error" /* ERROR */, {
   ...
   */
  "--demo:error:timeout": (_) => {
    throw createInvokerError("Operation timed out", "error" /* ERROR */, {
   ...
   */
  "--demo:debug:toggle": ({ targetElement }) => {
...
`;
    targetElement.textContent += newMode ? "Detailed command execution logs will be shown in console.\n" : "Debug logging disabled.\n";
  },
  /**
   * `--demo:debug:execute`: Demo command execution with debug info.
   * @example `<button command="--demo:debug:execute">`
   */
  "--demo:debug:execute": ({ targetElement }) => {
   ...
   */
  "--demo:clear": ({ targetElement }) => {
    targetElement.textContent = "";
  }
};
function registerDemoCommands(specificCommands) {
  if (!window.Invoker) {
 ...
 */
//# sourceMappingURL=demo-commands.js.map
